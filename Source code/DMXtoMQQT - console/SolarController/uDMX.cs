...
public class uDMX : IDisposable
{
    ...
    private bool SendCommand(Command command, short cvalue, short cindex, byte[] buffer)
    {
        ...
        // RIGTIG HARDWARE-LOGIK RETTELSE:
        if (buffer != null && buffer.Length > 0)
        {
            // Til SetChannelRange: Skal være lig med antallet af bytes i arrayet
            packet.Length = (short)buffer.Length;
        }
        else
        {
            // Til SetSingleChannel: uDMX firmwaren kræver 0 i pakkelængde, 
            // da selve lysstyrken allerede ligger i 'packet.Value'.
            packet.Length = 0;
            buffer = new byte[0]; // Undgå null-reference-fejl i LibUsbDotNet
        }

        // Send data og get the result - Nu matcher packet.Length og buffer.Length altid!
